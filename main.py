...
from hijri import (
    check_hilal, miladi_to_hicri, ramazan_basi, sevval_basi,
    ay_adi, get_sf, prev_conjunction, build_cal, DEFAULT_CRITERION
)
...
async def bugun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    result = await loop.run_in_executor(None, miladi_to_hicri, today, _criterion)
    gun, ay_adi_str, hay, hyil = result
    await update.message.reply_text(
        "Bugun  : " + str(today) + "\n"
        "Hicri  : " + str(gun) + " " + ay_adi_str + " " + str(hyil) + "\n"
        "Kriter : " + _criterion
    )
...
async def analiz_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if total > 0:
        ...
        lines.append("Hicbir yil hesaplanamadi.")
    for c in countries:
